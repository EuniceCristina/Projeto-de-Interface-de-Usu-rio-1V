...
export default function Form(){
   ...
   const adicionaTarefa = (e) => {
    ...
    function colecaoCores(){
        const tarefasColoridas = tarefas.filter(tarefa => tarefa.status === "Realizada");
        setCores(tarefasColoridas)
    }

    useEffect(() => 
    {colecaoCores()},[tarefas])
    ...
    return(
        ...
        {tarefas[0] ? <table border="1">
            <tr>
                ...
                <td>{tarefa.data}</td>
                <td>{tarefa.status === "Pendente" || tarefa.status === "Não realizada" ? <button onClick={() => concluirTarefa(tarefa.nome)}>Concluir</button> : <button onClick={() => abrirTarefa(tarefa.nome)} style={{backgroundColor : cores.some(c => c.nome === tarefa.nome) ? "green" : "gray"}}>Abrir</button>}</td>
